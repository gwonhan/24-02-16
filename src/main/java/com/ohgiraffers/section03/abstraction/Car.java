...
public class Car {
    ...
    private boolean isOn; // 초기에는 시동이 꺼진 false 상태이다. 부ㅡㄹ린 기본값은 풜스
    private  int speed ;
    /*시동이걸려있는 상태인 경우 할 일이 없어 시동이 걸려있지 않으면 시동을 간다.*/
    public void  startUp(){
        ...
        if (isOn){
            ...
            this.isOn = true;
            System.out.println(" 시동을 걸었습니다. 이제 출발을 할 준비가 되었습니다. ");
        }
    ...
    public void stop(){
        if (isOn){
            if (this.speed >0){
                ...
                System.out.println("차의 시동이 걸려있지 ㅇ낳습니다. 시동을 먼저 걸어주세요");

        }


    }
    /*시동이 이미 걸려있는 상태인 경우 시동을 끄고, 이미 꺼진 상태라면 끌 수 있다.*/
    public void turnOff(){
        if (isOn){
            if (speed > 0 ){
