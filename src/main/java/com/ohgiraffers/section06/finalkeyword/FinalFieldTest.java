...
public class FinalFieldTest {
    ...
    *  final 키워드를 사용할 수 있는 위치는 다양한 편이며 의미가 약간 다르지만, 결론적으론 변경 불가의 의미.
...
//    private final int nonstaticNum; // 0으로 초기화 이후 변경 불가
    ...
    /*필기. static 에서도 자바에서 지정한 기본값이 초기에 들어가기 때문에
...
//    private static final int STATIC_NUM;
    ...
    /*필기.
    *  생성자를 이용한 초기화는 불가능하다.
    *  생성자는 인스턴스가 생성되는 시점에 호출되기 떼문에 그 전에는 초기화가 일어나지 못한다.
    *  하지만 STATIC 은 프로그램이 START 될 때 할당되기 떄문에 초기화가 되지 않은 상태로 선언된 것과 돌일하여
    *  기본값으로 초기화 된 후값을 변경하지 못하기 때문에 에러가 발생한다.,*/
